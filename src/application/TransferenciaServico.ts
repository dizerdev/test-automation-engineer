...
export class TransferenciaServico {
  ...
  public async transferir(dto: TransferenciaDTO): Promise<string> {
    ...
    if (!contaOrigem) throw new NegocioErro("conta de origem não encontrada");

    if (!contaDestino) throw new NegocioErro("conta de destino não encontrada");

    const transferencia: TransferenciaValor = new TransferenciaValor();
    const recibo: Recibo = transferencia.transferir(
      contaOrigem,
      contaDestino,
      dto.valor
    );
