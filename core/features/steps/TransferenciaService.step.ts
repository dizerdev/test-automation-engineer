...
export class TransferenciaServicoStep {
  ...
  @given(
    "conta {string} com saldo {float} e a conta {string} com saldo {float}"
  )
  public dadaDuasContas(
    numeroOrigem: string,
    saldoOrigem: number,
  ...
  public quandoTransferirValores(
    numeroOrigem: string,
    valor: number,
    numeroDestino: string
  ): void {
    const transferenciaServico: TransferenciaServico = new TransferenciaServico(
      this.repositorio
    );
    const dto: TransferenciaDTO = new TransferenciaDTO(
      numeroOrigem,
      numeroDestino,
      valor
    );
  ...
  public async entaoSaldoDeveSer(
    numeroOrigem: string,
    resultadoOrigem: number,
    numeroDestino: string,
    resultadoDestino: number
  ): Promise<void> {
